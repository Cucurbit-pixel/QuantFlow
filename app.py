...
def dashboard():
    ...
    if options_alerts:
        ...
        medium = [a for a in options_alerts if a.get("level") == "medium"]
    ...
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; }}
            ...
            .options-title {{ font-size: 1.05rem; font-weight: 700; margin-bottom: 12px; color: #F5F3FF; }}
            .opt-level {{ font-size: 0.9rem; font-weight: 600; margin: 12px 0 6px 0; }}
            .opt-level.high {{ color: #F87171; }}
            .opt-level.medium {{ color: #FBBF24; }}
            .opt-level.low {{ color: #4ADE80; }}
            .opt-line {{ font-size: 0.85rem; color: #C4B5FD; line-height: 1.7; padding-left: 4px; }}
            .card {{
                background: #1A1229;
                border-radius: 16px;
                padding: 16px;
